...
# 2. 目标点（红色五角星，标注放在右上方）
ax.plot(target_x, target_y, 'r*', markersize=18, label='目标点')
ax.text(target_x + 1.0, target_y + 1.2, "目标点", color='red', fontsize=10, ha='left', weight='bold')
...
if len(intersections_12) == 2:
    ...
    for i, (ix, iy) in enumerate(intersections_12):
        ax.plot(ix, iy, 'o', markerfacecolor='none', markeredgecolor='purple', markersize=12, markeredgewidth=2)
        # 标注文字，避免与目标点重叠：第一个交点放在左下方，第二个放在右上方
        if i == 0:
            ax.text(ix - 1.5, iy - 1.2, f"歧义点{i+1}", color='purple', fontsize=9, ha='center')
        else:
            ax.text(ix + 1.0, iy + 1.2, f"歧义点{i+1}", color='purple', fontsize=9, ha='left')
    st.sidebar.success("✅ 站1与站2有两个交点（双解歧义）")

    # 判断三站唯一锁定：检查第三个圆是否通过其中一个交点
    unique_idx = None
    ...
    if unique_idx is not None:
        # 在正确交点处叠加一个绿色实心小圆点（内部填充绿色），表示唯一解
        ix, iy = intersections_12[unique_idx]
        ax.plot(ix, iy, 'o', color='lime', markersize=8, zorder=10)
        ax.text(ix, iy + 1.5, "✓ 唯一解", color='green', fontsize=10, ha='center', weight='bold')
...
ax.plot([], [], 'r*', label='目标点')
ax.plot([], [], 'o', markerfacecolor='none', markeredgecolor='purple', markersize=8, label='双站交点（歧义）')
...
with st.expander("📖 无源定位原理（点击展开）"):
    ...
    - **操作提示**：拖动左侧滑块调整侦察站或目标位置，观察两个紫色空心圆点（歧义点）以及第三个圆如何锁定正确交点。
    """)
